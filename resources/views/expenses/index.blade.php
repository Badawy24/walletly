...
<div class="container-fluid px-3 px-md-4">
    ...
    @if($expenses->count() > 0)
        <div class="card border-0">
            <div class="card-header balance-card-danger">
                        ...
                        <div class="card-body p-3">
                            <div class="d-flex justify-content-between align-items-start mb-2">
                                <div>
                                    ...
                                    <strong>{{ $expense->date->format('D, M d, Y') }}</strong>
                                </div>
                                <div class="text-end">
                                    <span class="badge balance-card-danger rounded-pill mb-1">
                                        {{ ucfirst($expense->category) }}
                                    </span>
                                    <div>
                                        <span class="fw-bold text-danger-color amount-large">-{{ number_format($expense->amount, 2) }} EGP</span>
                                    </div>
                                </div>
                            </div>
                            @if($expense->notes)
                                <div class="mt-2 pt-2 border-top">
                                    ...
                                    <span class="text-muted small">{{ $expense->notes }}</span>
                                </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            @foreach($expenses as $expense)
                                <tr>
                                    <td class="ps-4">
                                        <strong>{{ $expense->date->format('D, M, d') }}</strong>
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-muted small pe-4">{{ $expense->notes ?? '-' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
